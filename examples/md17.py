...
param_path = 'examples/configs/md17_ef_config.json'
...
Ntrain = args.ntrain
Nvalid = args.nvalid
Ntest = args.ntest if hasattr(args,'ntest') else Nmols - Ntrain - Nvalid
...
if args.model == 'EGNN':
    rep_model = EGNN(in_node_nf=args.max_atom_type * (args.charge_power + 1), hidden_nf=args.hidden_dim, out_node_nf=args.hidden_dim, in_edge_nf=0,
                    n_layers=args.n_layers, use_vel=False)
elif args.model == 'RF':
...
task = EnergyForcePrediction(rep=rep_model, rep_dim=args.hidden_dim, decoder_type=args.decoder, mean=mean, std=std, loss='MAE')
