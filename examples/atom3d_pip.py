...
param_path = 'examples/configs/atom3d_pip/egnn.yaml'
args = load_params(param_path)
set_seed(args.trainer.seed)

transform = LEP_Transform()
datasets = {
    'train': Atom3DDataset(root=args.data.data_path, task='pip', split='train', dataset_arg='DIPS', transform=transform),
    'val': Atom3DDataset(root=args.data.data_path, task='pip', split='val', dataset_arg='DIPS', transform=transform),
    'test': Atom3DDataset(root=args.data.data_path, task='pip', split='test', dataset_arg='DIPS', transform=transform)
}
