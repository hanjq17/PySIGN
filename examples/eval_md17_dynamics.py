...
sys.path.append('./')
from airgeom.dataset import MD17, MD17_Dynamics
...
test_dataset = dataset[args.vel_step + 10000: args.vel_step + 20000]
...
if args.model == 'EGNN':
    rep_model = EGNN(in_node_nf=args.max_atom_type * (args.charge_power + 1), hidden_nf=args.hidden_dim,
                     out_node_nf=args.hidden_dim, in_edge_nf=0,
    ...
    rep_model = SchNet(in_node_nf=args.max_atom_type * (args.charge_power + 1), out_node_nf=args.hidden_dim,
                       hidden_nf=args.hidden_dim)

elif args.model == 'PaiNN':
    rep_model = PaiNN(max_z=args.max_atom_type * (args.charge_power + 1), n_atom_basis=args.hidden_dim,
                      n_interactions=args.n_layers)

elif args.model == 'ET':
    rep_model = EquivariantTransformer(max_z=args.max_atom_type * (args.charge_power + 1),
