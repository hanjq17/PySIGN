...
class SchNet(nn.Module):
    ...
    """
    def __init__(self, in_node_nf: int, hidden_nf: int=128, num_filters: int=128, num_interactions: int=6,
                 num_gaussians: int=50, cutoff: float=10.0, max_num_neighbors: int=32):
        super().__init__()
        ...
        for _ in range(num_interactions):
            self.interactions.append(InteractionBlock(hidden_nf, num_gaussians,
                                                      num_filters, cutoff))
    ...
    def forward(self, data):
        ...
        for interaction in self.num_interactions:
