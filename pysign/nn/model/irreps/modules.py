...
class GConvSE3(nn.Module):
    ...
    def forward(self, node_features, edge_index, edge_attr, r, basis, **kwargs):
        if edge_attr is not None:
            feat = torch.cat([edge_attr, r], -1)
        else:
            feat = torch.cat([r, ], -1)
        edata = {}
        for (mi, di) in self.f_in.structure:
            for (mo, do) in self.f_out.structure:
                etype = f'({di},{do})'
                edata[etype] = self.kernel_unary[etype](feat, basis)
...
def get_basis(dis, max_degree, compute_gradients):
    ...
    Returns:
        ...
        channels
    """
    assert compute_gradients is False

    with torch.no_grad():
        # cloned_d = torch.clone(G.edata['d'])
        cloned_d = torch.clone(dis)
...
def get_r(dis):
    ...
    return torch.sqrt(torch.sum(dis ** 2, -1, keepdim=True))


def get_basis_and_r(dis, max_degree, compute_gradients=False):
    ...
    """
    basis = get_basis(dis, max_degree, compute_gradients)
    r = get_r(dis)
    return basis, r
...
class GConvSE3Partial(nn.Module):
    ...
    def forward(self, node_features, edge_index, edge_attr, r, basis, x):
        if edge_attr is not None:
            feat = torch.cat([edge_attr, r], -1)
        else:
            feat = torch.cat([r, ], -1)
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, edge_index, k=None, q=None):
        edata = {}
        for m, d in self.f_value.structure:
            edata[f'v{d}'] = v[f'{d}'].view(-1, self.n_heads, m // self.n_heads, 2 * d + 1)
        edge_key = fiber2head(k, self.n_heads, self.f_key, squeeze=True)  # [edges, heads, channels](?)
        node_q = fiber2head(q, self.n_heads, self.f_key, squeeze=True)  # [nodes, heads, channels](?)
        edge_e = torch.sum(edge_key * node_q[edge_index[1]], dim=-1)
        n_edges = edge_index.shape[1]
        edge_e = edge_e.view(n_edges, self.n_heads)
        edge_e = edge_e / np.sqrt(self.f_key.n_features)

        edge_a = softmax(edge_e, edge_index[0])  # TODO: need check
...
class GSE3Res(nn.Module):
    ...
    def forward(self, node_features, edge_index, edge_attr, r, basis, x):
        # Embeddings
        v = self.GMAB['v'](node_features, edge_index, edge_attr, r, basis, x)
        k = self.GMAB['k'](node_features, edge_index, edge_attr, r, basis, x)
        q = self.GMAB['q'](node_features)

        # Attention
        z = self.GMAB['attn'](v, edge_index, k=k, q=q)

        if self.skip == 'cat':
            z = self.cat(z, node_features)
            z = self.project(z)
        elif self.skip == 'sum':
            # Skip + residual
            z = self.project(z)
            z = self.add(z, node_features)
        return z
