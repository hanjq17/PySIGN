...
class Trainer(object):
    def __init__(self, dataloaders, task, args, device, lower_is_better=True, verbose=True):
        ...
        self.model_saver = SavingHandler(self.task, save_path=args.model_save_path,
                                         lower_is_better=lower_is_better, max_instances=3)
    ...
    def train_epoch(self):
        ...
        for step, batch_data in enumerate(train_loader):
            ...
            if self.verbose:
                if step % 20 == 0:
                    print('Step: {:3d} | Train Loss: {:.3f}'.format(step, loss.item()))
    ...
    def loop(self):
        for ep in range(self.args.epoch):
            ...
            print('Epoch: {:3d} | Train Loss: {:.3f} | Val Loss {:.3f} | Test Loss {:.3f}'.format(ep, train_loss,
                                                                                                  val_loss, test_loss))
