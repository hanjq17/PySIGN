...
class GConvSE3(nn.Module):
    ...
    """
    def __init__(self, f_in, f_out, self_interaction: bool=False, edge_dim: int=0, act_fn=nn.ReLU()):
        ...
        for (mi, di) in self.f_in.structure:
            for (mo, do) in self.f_out.structure:
                self.kernel_unary[f'({di},{do})'] = PairwiseConv(di, mi, do, mo, edge_dim=edge_dim, act_fn=act_fn)
...
class PairwiseConv(nn.Module):
    ...
    def __init__(self, degree_in: int, nc_in: int, degree_out: int, 
                 nc_out: int, edge_dim: int=0, act_fn=nn.ReLU()):
...
class GSE3Res(nn.Module):
    ...
    def __init__(self, f_in: Fiber, f_out: Fiber, edge_dim: int=0, div: float=4,
                 n_heads: int=1, act_fn=nn.ReLU(), learnable_skip=True):
        super().__init__()
        self.f_in = f_in
        self.f_out = f_out
        self.div = div
        ...
        # Projections
        self.GMAB['v'] = GConvSE3Partial(f_in, self.f_mid_out, edge_dim=edge_dim, act_fn=act_fn)
        self.GMAB['k'] = GConvSE3Partial(f_in, self.f_mid_in, edge_dim=edge_dim, act_fn=act_fn)
